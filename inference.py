...
def run_inference(image, model, sess, mode, object_map):
	"""
	...
	if mode != 'video':
		...
		video_text = 'CNN inference + NMS speed: %.2f fps' % (1/(time.time() - t0),)
	...
	for box in boxes:
		...
		# Annotate image
		image = cv2.rectangle(image, tuple(box_coords[:2]), tuple(box_coords[2:]), (0,255,0), 3)
		label_str = '%s %.2f' % (object_map[cls], cls_prob)
		image = cv2.putText(image, label_str, (box_coords[0], box_coords[1]), 0, 0.5, (0,255,0), 1, cv2.LINE_AA)
		if mode == 'video':
			image = cv2.putText(image, video_text, (20, 20), 0, 0.5, (0,255,0), 1, cv2.LINE_AA)
...
def generate_output(input_files, mode):
	...
	"""
	# First, load mapping from integer class ID to object name string
	object_map = {}
	with open('objectnames.csv', 'r') as f:
		for line in f:
			line = line[:-1]  # strip newline at the end
			object_id, object_name = line.split(',')
			object_map[int(object_id)] = object_name
	object_map[0] = 'background'  # class ID 0 reserved for background class
	...
	with tf.Graph().as_default(), tf.Session() as sess:
		...
		if mode == 'image':
			for image_file in input_files:
				...
				image = run_inference(image_orig, model, sess, mode, object_map)
			...
			for video_file in input_files:
				...
				video = video.fl_image(lambda x: run_inference(x, model, sess, mode, object_map))
